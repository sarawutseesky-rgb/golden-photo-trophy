...
// Verifies an anonymous VISITOR (not logged in, not the owner) sees the
// Milestone stars card with the correct filled / empty star badges and the
// continuous timer status text ("Held #1 for Xd · next ★ at Yd" or the
// not-holding fallback), and that these values stay stable across 4
// consecutive hard refreshes.
test.describe("Photo detail — guest Milestone stars stable across 4 hard refreshes", () => {
  test("milestone star badges and timer status do not change after refreshes", async ({ page, context }) => {
    ...
    // Pick first photo from public feed.
    await page.goto("/");
